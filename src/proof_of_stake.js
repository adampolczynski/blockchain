...
const Account = require('./wallet/accounts')
...
class Block {
    ...
    static createBlock(lastBlock, data, wallet) {
        console.log('createBlock wallet: ', wallet)
        let hash
        let timestamp = Date.now()
        const lastHash = lastBlock.hash
        hash = Block.hash(timestamp, lastHash, data)

        // need to do exclusion for first node in the chain
        // get the validators public key
        let validator = wallet && wallet.getPublicKey()

        // Sign the block
        let signature = wallet && Block.signBlockHash(hash, wallet)
...
class Blockchain {
    ...
    getBalance(publicKey) {
        return 1000 //this.accounts.getBalance(publicKey)
